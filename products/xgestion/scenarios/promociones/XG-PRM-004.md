---
{"id":"XG-PRM-004","title":"Recalcular el descuento del 50 % en la segunda unidad","product":"xgestion","module":"promociones","tags":["xgestion","regression","promociones","escritura"],"status":"implemented","test":"products/xgestion/suites/promociones.robot","seed":"catalogo-comercial-v1"}
---
...
El vendedor comprueba la promoción al cargar un artículo, corregir su cantidad y cobrarlo una sola vez después de cancelar el primer intento de cobro.
...
- Baseline restaurado y seed `catalogo-comercial-v1` aplicado antes de abrir el JAR. El runner lo prepara automáticamente al seleccionar este caso por su ID, grupo o menú; `--seed` también permite pedirlo explícitamente. Ejemplo público `2DA50-Q3`, artículo `QA-SEED-2DA50`, precio base $1.000 ARS y stock suficiente.
- 50 % de descuento en la segunda unidad de cada par; la tercera conserva su precio base.
...
```powershell
.\qa.cmd run --product xgestion --scenario XG-PRM-004 --seed catalogo-comercial-v1 --log-level DEBUG
```
...
2. Cargar `QA-SEED-2DA50` con cantidad **1**. **Esperado:** una sola línea del artículo correcto; precio unitario $1.000, subtotal bruto $1.000, oferta $0 y total neto **$1.000 ARS**.
3. Seleccionar esa misma línea, abrir su editor mediante **Ctrl+E**, cambiar cantidad a **3** y guardar. **Esperado:** se conserva la identidad del artículo y una sola línea; precio base $1.000, subtotal bruto $3.000, oferta $500 y total neto **$2.500 ARS**. La edición no agrega otro producto ni aplica dos veces el beneficio.
4. Abrir el cobro y cancelarlo antes de confirmar. **Esperado:** vuelve la venta con la cantidad 3 y el neto $2.500; todavía no se registra venta, cobro ni movimiento de stock/caja.
5. Retomar el cobro, entregar el importe exacto **$2.500 ARS** y confirmar una vez. **Esperado:** operación cerrada por ese importe y vuelto cero.
6. Contrastar el resultado guardado. **Esperado:** una sola venta y un único cobro por $2.500; stock del artículo **−3** y caja **+$2.500**. No hay descuentos adicionales ni duplicados del intento cancelado.
...
Fuente ERP: commit `925589278503f2d339beeb0a79f773c605512dd8`; `src/ModuloVentas/Vistas/FormVenta.java` y `src/ModuloVentas/Entidades/TicketVenta.java` para presentación/persistencia. Regla de referencia: `test/ModuloFinanzas/Entidades/OfertaCantidadPorcentajeTest.java`. Datos y expectativa estática: [pricing.py](../../seeds/pricing.py), ejemplo `2DA50-Q3`. Las referencias orientan el caso; no demuestran equivalencia ni aceptación del JAR.
