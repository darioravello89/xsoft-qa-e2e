---
{"id":"XG-PRM-001","title":"Aplicar una promoción porcentual y recalcular la cantidad","product":"xgestion","module":"promociones","tags":["xgestion","regression","promociones","escritura"],"status":"implemented","test":"products/xgestion/suites/promociones.robot","seed":"catalogo-comercial-v1"}
---
...
- Baseline restaurado y seed `catalogo-comercial-v1` aplicado antes de abrir el JAR. El runner lo prepara automáticamente al seleccionar este caso por su ID, grupo o menú; `--seed` también permite pedirlo explícitamente. Ejemplo público `PCT-Q3`, artículo `QA-SEED-PCT`, precio base $1.000 ARS y stock suficiente.
- 10 % de descuento sobre el precio base de cada unidad.
...
```powershell
.\qa.cmd run --product xgestion --scenario XG-PRM-001 --seed catalogo-comercial-v1 --log-level DEBUG
```
...
2. Cargar `QA-SEED-PCT` con cantidad **1**. **Esperado:** una sola línea del artículo correcto; precio unitario $1.000, subtotal bruto $1.000, oferta $100 y total neto **$900 ARS**.
3. Seleccionar esa misma línea, abrir su editor mediante **Ctrl+E**, cambiar cantidad a **3** y guardar. **Esperado:** se conserva la identidad del artículo y una sola línea; precio base $1.000, subtotal bruto $3.000, oferta $300 y total neto **$2.700 ARS**. La edición no agrega otro producto ni aplica dos veces el beneficio.
4. Abrir el cobro y cancelarlo antes de confirmar. **Esperado:** vuelve la venta con la cantidad 3 y el neto $2.700; todavía no se registra venta, cobro ni movimiento de stock/caja.
5. Retomar el cobro, entregar el importe exacto **$2.700 ARS** y confirmar una vez. **Esperado:** operación cerrada por ese importe y vuelto cero.
6. Contrastar el resultado guardado. **Esperado:** una sola venta y un único cobro por $2.700; stock del artículo **−3** y caja **+$2.700**. No hay descuentos adicionales ni duplicados del intento cancelado.
...
Fuente ERP: commit `925589278503f2d339beeb0a79f773c605512dd8`; `src/ModuloVentas/Vistas/FormVenta.java` y `src/ModuloVentas/Entidades/TicketVenta.java` para presentación/persistencia. Regla de referencia: `test/ModuloFinanzas/Entidades/OfertaPorcentajeTest.java`. Datos y expectativa estática: [pricing.py](../../seeds/pricing.py), ejemplo `PCT-Q3`. Las referencias orientan el caso; no demuestran equivalencia ni aceptación del JAR.
