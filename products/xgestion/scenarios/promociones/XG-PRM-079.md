---
{"id":"XG-PRM-079","title":"Permitir o rechazar el descuento manual sobre artículos en oferta","product":"xgestion","module":"promociones","tags":["xgestion","regression","promociones","promociones-condiciones","descuentos","permisos","escritura"],"status":"implemented","test":"products/xgestion/suites/ofertas/condiciones.robot","seed":"catalogo-comercial-v1"}
---
...
Permitir o rechazar el descuento manual sobre artículos en oferta durante una venta, con importes y efectos observables por el vendedor o cajero.
...
```powershell
.\qa.cmd run --product xgestion --scenario XG-PRM-079 --seed catalogo-comercial-v1 --log-level DEBUG
```
...
## Variante 1: general-off

Permiso general de descuento manual deshabilitado.

### Productos preparados

| Referencia | Código | Precio base | Unidad | Familia / subfamilia / sector | Marca |
| --- | --- | ---: | --- | --- | --- |
| A | `QA-PRM-079-A` | $1.000,00 | unidad | 988900 / 988900 / 980001 | QA-SEED |
| B | `QA-PRM-079-B` | $1.000,00 | unidad | 988900 / 988900 / 980001 | QA-SEED |

### Ofertas preparadas

| Oferta | Incluye | Regla | Agrupación | Estado / vigencia inclusiva | Medios habilitados |
| --- | --- | --- | --- | --- | --- |
| `QA-PRM-079` | Familia `988900` | 10 % sobre el importe | ON | Activa; hoy -365 días a hoy +365 días | Todos |
...
| 2 | Cargar 1 u de `QA-PRM-079-A` por código. | `QA-PRM-079-A` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal | Total **$900,00**. |
| 3 | Cargar 1 u de `QA-PRM-079-B` por código. | `QA-PRM-079-A` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal<br>`QA-PRM-079-B` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal | Total **$1.800,00**. |
| 4 | Abrir Ctrl+E en `QA-PRM-079-A` y comprobar que no permite editar $100,00 de descuento manual. | `QA-PRM-079-A` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal<br>`QA-PRM-079-B` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal | Total **$1.800,00**. Canasta y descuentos sin cambios; control de importe no editable. |
...
## Variante 2: offers-off

Permiso general habilitado y permiso de descuento sobre ofertas deshabilitado.
...
| 2 | Cargar 1 u de `QA-PRM-079-A` por código. | `QA-PRM-079-A` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal | Total **$900,00**. |
| 3 | Cargar 1 u de `QA-PRM-079-B` por código. | `QA-PRM-079-A` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal<br>`QA-PRM-079-B` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal | Total **$1.800,00**. |
| 4 | Abrir Ctrl+E en `QA-PRM-079-A`, guardar $100,00 de descuento manual total. | `QA-PRM-079-A` × 1 u: precio $1.000,00; automático $90,00; manual $100,00; neto $810,00; lista aplicada: precio normal<br>`QA-PRM-079-B` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal | Total **$1.710,00**. |
| 5 | Abrir cobro en efectivo del perfil QA, ingresar $1.710,00 y cancelar antes de confirmar. | `QA-PRM-079-A` × 1 u: precio $1.000,00; automático $90,00; manual $100,00; neto $810,00; lista aplicada: precio normal<br>`QA-PRM-079-B` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal | Total **$1.710,00**. Conserva toda la canasta; sin venta cobrada, pago ni movimientos. |
| 6 | Retomar el cobro en efectivo del perfil QA, ingresar $1.710,00 y confirmar una sola vez. | `QA-PRM-079-A` × 1 u: precio $1.000,00; automático $90,00; manual $100,00; neto $810,00; lista aplicada: precio normal<br>`QA-PRM-079-B` × 1 u: precio $1.000,00; automático $100,00; manual $0,00; neto $900,00; lista aplicada: precio normal | Total **$1.710,00**. Una venta y un cobro en efectivo del perfil QA; vuelto cero. Stock: `QA-PRM-079-A` −1 u; `QA-PRM-079-B` −1 u. |

## Variante 4: allowed-warning-off

- Permisos general y sobre ofertas habilitados; descuento manual por importe total de línea.
- Aviso de riesgo del descuento manual: ausente durante la ventana de observación calibrada.
...
- Dos productos nuevos A/B de $1.000 en una misma familia con 10% agrupado, sin mínimo. Tres perfiles independientes: permiso general OFF; permiso general ON y permiso sobre ofertas OFF; ambos ON. Descuento manual de línea A=$100, sin descuentos globales/cliente/pago. Variante aviso de riesgo ON/OFF. Todos los perfiles y los selectores del editor están pendientes de preparar.
...
| --- | --- |
| Cargar A×1 y B×1 en cada perfil. | Ofertas $100 por línea; total $1.800. |
| Intentar editar el descuento manual de A en los dos perfiles restringidos. | No permite guardar $100 de descuento manual; mantiene total $1.800 y la operación editable. |
| Con ambos permisos ON, guardar $100 de descuento manual en A. | Base restante de A=$900, oferta automática agrupada 10%=$90, neto A=$810; B=$900; total $1.710. |
| Repetir con aviso ON y OFF en perfiles independientes. | El aviso aparece según configuración y no cambia el total de $1.710 ni duplica descuentos. |
| Cancelar el cobro y retomarlo por $1.710 en el perfil permitido. | Sin movimientos al cancelar; una venta al confirmar y ambos descuentos identificados por separado. |

## Dependencias para automatizar

Verificar UI de permiso, aviso y descuento como importe total del renglón. La cifra esperada usa explícitamente oferta porcentual agrupada sobre bruto menos manual; otros órdenes/descuentos globales requieren otra ficha.
...
Rutas relativas a XGestion2: `test/ModuloVentas/Vistas/FormVentaDetalleDescuentoOfertaPolicyTest.java`; `src/ModuloFinanzas/Entidades/OfertaAplicacionCalculador.java`; `src/ModuloFinanzas/Entidades/OfertaLineaService.java`.
