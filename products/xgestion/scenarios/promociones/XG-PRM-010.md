---
{"id":"XG-PRM-010","title":"Cobrar a $800 la unidad desde dos unidades por producto","product":"xgestion","module":"promociones","tags":["xgestion","regression","promociones","promociones-alcances","ofertas-producto","escritura"],"status":"implemented","test":"products/xgestion/suites/ofertas/producto.robot","seed":"catalogo-comercial-v1"}
---
...
El vendedor comprueba precio de $800 desde 2 unidades por producto, distingue los productos incluidos de los ajenos y conserva el importe correcto al corregir cantidades y retomar un cobro.

<!-- BEGIN GENERATED OFFER JOURNEY -->

## Recorrido de automatización

Estado del catálogo: **implemented**. Automatización catalogada en [`producto.robot`](../../suites/ofertas/producto.robot). Validación real: **pendiente**; requiere ejecutar el JAR exacto en el laboratorio calibrado.
...
```powershell
.\qa.cmd run --product xgestion --scenario XG-PRM-010 --seed catalogo-comercial-v1 --log-level DEBUG
```
...
| --- | --- | ---: | --- | --- | --- |
| A | `QA-PRM-010-A` | $1.000,00 | unidad | 980001 / 980001 / 980001 | QA-SEED |
| E | `QA-PRM-010-E` | $1.000,00 | unidad | 980001 / 980001 / 980001 | QA-SEED |

### Ofertas preparadas

| Oferta | Incluye | Regla | Agrupación | Estado / vigencia inclusiva | Medios habilitados |
| --- | --- | --- | --- | --- | --- |
| `QA-PRM-010` | Producto `QA-PRM-010-A` | Desde 2: precio final $800,00 por unidad | OFF | Activa; hoy -365 días a hoy +365 días | Todos |
...
| 2 | Cargar 1 u de `QA-PRM-010-E` por código. | `QA-PRM-010-E` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal | Total **$1.000,00**. |
| 3 | Cargar 1 u de `QA-PRM-010-A` por código. | `QA-PRM-010-E` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal<br>`QA-PRM-010-A` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal | Total **$2.000,00**. |
| 4 | Seleccionar `QA-PRM-010-A`, abrir Ctrl+E, dejar 2 u y guardar. | `QA-PRM-010-E` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal<br>`QA-PRM-010-A` × 2 u: precio $1.000,00; automático $400,00; manual $0,00; neto $1.600,00; lista aplicada: precio normal | Total **$2.600,00**. |
| 5 | Seleccionar `QA-PRM-010-A`, abrir Ctrl+E, dejar 3 u y guardar. | `QA-PRM-010-E` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal<br>`QA-PRM-010-A` × 3 u: precio $1.000,00; automático $600,00; manual $0,00; neto $2.400,00; lista aplicada: precio normal | Total **$3.400,00**. |
| 6 | Seleccionar `QA-PRM-010-A`, abrir Ctrl+E, dejar 1 u y guardar. | `QA-PRM-010-E` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal<br>`QA-PRM-010-A` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal | Total **$2.000,00**. |
| 7 | Seleccionar `QA-PRM-010-A`, abrir Ctrl+E, dejar 2 u y guardar. | `QA-PRM-010-E` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal<br>`QA-PRM-010-A` × 2 u: precio $1.000,00; automático $400,00; manual $0,00; neto $1.600,00; lista aplicada: precio normal | Total **$2.600,00**. |
| 8 | Abrir cobro en efectivo del perfil QA, ingresar $2.600,00 y cancelar antes de confirmar. | `QA-PRM-010-E` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal<br>`QA-PRM-010-A` × 2 u: precio $1.000,00; automático $400,00; manual $0,00; neto $1.600,00; lista aplicada: precio normal | Total **$2.600,00**. Conserva toda la canasta; sin venta cobrada, pago ni movimientos. |
| 9 | Retomar el cobro en efectivo del perfil QA, ingresar $2.600,00 y confirmar una sola vez. | `QA-PRM-010-E` × 1 u: precio $1.000,00; automático $0,00; manual $0,00; neto $1.000,00; lista aplicada: precio normal<br>`QA-PRM-010-A` × 2 u: precio $1.000,00; automático $400,00; manual $0,00; neto $1.600,00; lista aplicada: precio normal | Total **$2.600,00**. Una venta y un cobro en efectivo del perfil QA; vuelto cero. Stock: `QA-PRM-010-E` −1 u; `QA-PRM-010-A` −2 u. |
...
**Pendiente de automatización (`planned`), sin ejecución real registrada.** Etapa 2, grupo `promociones-alcances` / `ofertas-producto`; prioridad **P1**. Consultar el [índice de pendientes](../../docs/promociones-pendientes.md). Esta ficha no tiene Robot ni se ofrece como caso ejecutable.
...
- Datos disponibles en `catalogo-comercial-v1`: A = `QA-SEED-MIN-PRECIO` (artículo 980107); oferta exclusiva de ese producto; E = `QA-SEED-NORMAL` (980001), ajeno y sin ofertas. Ejemplo público `MIN-PRECIO-Q2` en [pricing.py](../../seeds/pricing.py).
- A y E son productos distintos aun cuando compartan familia, subfamilia o sector: una oferta por producto no se extiende a esas clasificaciones.
...
| 3 | Cargar A (`QA-SEED-MIN-PRECIO`), cantidad 1. | A: bruto $1.000,00, descuento $0,00, neto $1.000,00. E continúa en $1.000,00; total de venta $2.000,00. |
...
| 7 | Editar A nuevamente a cantidad 2. | A queda en $1.600,00 y E en $1.000,00; total $2.600,00. Se mantienen identidades y una línea por producto. |
| 8 | Abrir el cobro y cancelarlo antes de confirmar. | La venta vuelve con el mismo total $2.600,00 y las mismas líneas. No queda venta confirmada, cobro ni movimiento de stock/caja. |
| 9 | Retomar el cobro, entregar $2.600,00 en efectivo y confirmar una sola vez. | Venta cerrada por $2.600,00, vuelto cero; no cobra otra vez el intento cancelado. |
| 10 | Consultar el comprobante interno y contrastar los efectos de la operación. | Una venta y un cobro; bruto $3.000,00, oferta $400,00, neto $2.600,00. Stock A −2, E −1; caja +$2.600,00. |
...
No cubre fracciones, listas, otros medios de pago, ofertas agrupadas, superposiciones, fiscalización ni impresión. El límite por cantidad corresponde únicamente a A; sumar E no ayuda a alcanzarlo.
