---
{"id":"XG-PRM-003","title":"Recalcular una promoción 2x1 al pasar de una a tres unidades","product":"xgestion","module":"promociones","tags":["xgestion","regression","promociones","escritura"],"status":"implemented","test":"products/xgestion/suites/promociones.robot","seed":"catalogo-comercial-v1"}
---
...
- Baseline restaurado y seed `catalogo-comercial-v1` aplicado antes de abrir el JAR. El runner lo prepara automáticamente al seleccionar este caso por su ID, grupo o menú; `--seed` también permite pedirlo explícitamente. Ejemplo público `2X1-Q3`, artículo `QA-SEED-2X1`, precio base $1.000 ARS y stock suficiente.
- 2x1: una unidad no obtiene descuento; tres unidades pagan dos.
...
```powershell
.\qa.cmd run --product xgestion --scenario XG-PRM-003 --seed catalogo-comercial-v1 --log-level DEBUG
```
...
2. Cargar `QA-SEED-2X1` con cantidad **1**. **Esperado:** una sola línea del artículo correcto; precio unitario $1.000, subtotal bruto $1.000, oferta $0 y total neto **$1.000 ARS**.
3. Seleccionar esa misma línea, abrir su editor mediante **Ctrl+E**, cambiar cantidad a **3** y guardar. **Esperado:** se conserva la identidad del artículo y una sola línea; precio base $1.000, subtotal bruto $3.000, oferta $1.000 y total neto **$2.000 ARS**. La edición no agrega otro producto ni aplica dos veces el beneficio.
4. Abrir el cobro y cancelarlo antes de confirmar. **Esperado:** vuelve la venta con la cantidad 3 y el neto $2.000; todavía no se registra venta, cobro ni movimiento de stock/caja.
5. Retomar el cobro, entregar el importe exacto **$2.000 ARS** y confirmar una vez. **Esperado:** operación cerrada por ese importe y vuelto cero.
6. Contrastar el resultado guardado. **Esperado:** una sola venta y un único cobro por $2.000; stock del artículo **−3** y caja **+$2.000**. No hay descuentos adicionales ni duplicados del intento cancelado.
