...
Extender `locators.elements` con `sale.lines`, `sale.customer`, `sale.price_list`, `sale.unknown_notice`, `payment.total`, `payment.change`, `payment.cancel`, `sale.cancel_reject`, `editor.quantity`, `editor.save` y `editor.product`; para `unknown_notice: dialog`, agregar también `sale.unknown_dismiss`. Si el JAR implementa la edición accesible, agregar `sale.edit` y `editor.cancel` y verificar la feature `ventas-teclado-v1`. Cada alias conserva `window` y `query` observados y debe identificar un único control.
...
Después de verificar la extensión con el JAR, agregar `ventas-etapa1` a la lista `locators.calibration.verified_features`, además de `status: verified`, hash, persona, fecha y versión JAB. No marcar ese alcance solamente porque los selectores iniciales estaban verificados. Para el mecanismo nuevo, agregar `ventas-teclado-v1` sólo tras verificar sobre ese mismo SHA256 `sale.edit`, `editor.cancel`, selección de fila, foco en Cantidad, cancelación con restauración y reapertura con `Ctrl+E`. Sin esa feature XG-VEN-003/007 conservan la ruta legada por doble clic. No copiar una aprobación anterior ni habilitar la feature por inspección de fuente.
