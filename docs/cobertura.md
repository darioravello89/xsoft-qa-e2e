...
La foto actual distingue **315 fichas: 99 implementadas y 216 planificadas**, con **0 validaciones reales registradas en este mapa público**. Las implementadas son cinco de smoke, nueve de ventas, 82 de promociones y tres circuitos; las pendientes son PRM-077/078 (paquete de varios contextos), 24 remitos de compra, 40 Restobar, 28 listas de precios y 58 de circuitos críticos (clientes/proveedores/cuotas, Libro Diario/caja, conciliación, inventario y respaldos), más 50 del complemento crítico y FIN-012 pendiente por accesibilidad (ACC-010). Ese cero no afirma que nadie haya ejecutado pruebas en privado: el generador no lee reportes de QA. Las 20 referencias R01–R20 de Restobar y los 26 ejemplos de cálculo del seed tienen su propio espacio; no se suman a las 315 fichas ni a los 99 casos implementados. Cada R enlaza los IDs XG-RES relacionados en el roadmap. Veinte ejemplos seed enlazan fichas implementadas y seis enlazan fichas de listas pendientes; los 26 tienen referencia, sin acreditar el JAR. Ningún vínculo otorga PASS ni garantiza todos los datos de la ficha. La mejora de accesibilidad del editor se registra en un backlog independiente; la especificación inicial del mapa conserva sus conteos históricos y las fuentes actuales determinan cada regeneración.
...
El total pendiente incluye además **13 fichas de [atajos de listados](../products/xgestion/docs/atajos-listados.md)**, KEY-001..013. Filtrar Grupos para QA por `atajos-listados` o `filtros-listados` (ocho pantallas). El nuevo Enter de cuentas de clientes existe en fuente, pero ACC-010/FIN-012 siguen pendientes de validarlo en el JAR.
