...
6. Para los siete recorridos nuevos de Venta, verificar también `sales_journeys`, grilla completa, columnas, defaults, aviso de código ausente, cancelación/vuelto y el editor. Usar botón/`Ctrl+E` sólo cuando `ventas-teclado-v1` haya sido observado para el SHA256 exacto; conservar el doble clic identificado por JAB para paquetes anteriores. El [backlog CSV](../products/xgestion/docs/backlog-accesibilidad.csv) distingue implementación técnica de validación real, sin coordenadas fijas ni selectores inventados. Seguir la [calibración de Venta cotidiana](../products/xgestion/docs/calibracion.md); declarar `ventas-etapa1` en `calibration.verified_features` únicamente después de comprobar esa extensión. Los siete casos iniciales no habilitan por sí solos los siete nuevos.
